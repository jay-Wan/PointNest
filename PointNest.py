...
class Network:
        ...
        for j in range(self.config.num_layers):
            if j == 0:
                ...
                for i in range(self.config.num_layers - j):
                    # encoding[(i, j-1)]
                    f_encoder_i_j = self.feature_agg_block(feature, inputs['xyz'][i], inputs['neigh_idx'][i], d_out[i], 'Encoder_layer_' + str(j) + str(i), is_training)
                ...
                for i in range(self.config.num_layers - j):
                    ...
                    if i >= 1:
                        ...
                        if j + i == self.config.num_layers - 1 and i != 3:
                            ...
                            f_3 = jin[0][i]
                            feature = tf.concat([feature, f_3], axis=3)
                    f_decoder_i_j = self.feature_agg_block(feature, inputs['xyz'][i], inputs['neigh_idx'][i], d_out[i], 'Decoder_layer_' + str(j) + str(i), is_training)
                    # f_decoder_i_j = helper_tf_util.conv2d_transpose(feature, d_out[i], [1, 1],
                    #     'Decoder_layer_' + str(j)+str(i), [1, 1], 'VALID', bn=True,
    ...
    def feature_agg_block(self, feature, xyz, neigh_idx, d_out, name, is_training):
        f_pc = helper_tf_util.conv2d(feature, d_out, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        GCM_1 = self.graph_conv(xyz, f_pc, neigh_idx, d_out, name + 'GCM1', is_training)
        GCM_2 = self.graph_conv(xyz, GCM_1+f_pc, neigh_idx, d_out, name + 'GCM2', is_training)
        return tf.nn.leaky_relu(f_pc+GCM_1+GCM_2)

    def graph_conv(self, xyz, feature, neigh_idx, d_out, name, is_training):
        d_in = feature.get_shape()[-1].value
        f_xyz = self.relative_pos_encoding(xyz, neigh_idx)
        f_xyz = helper_tf_util.conv2d(f_xyz, d_in, [1, 1], name + 'mlp', [1, 1], 'VALID', True, is_training)
        f_neighbours = self.gather_neighbour(tf.squeeze(feature, axis=2), neigh_idx)
        f_tile = tf.tile(feature, [1, 1, tf.shape(neigh_idx)[-1], 1])
        f_concat = tf.concat([f_neighbours-f_tile], axis=-1)
        f_max = tf.reduce_max(f_concat, axis=-2, keep_dims=True)
        f_pc_agg = helper_tf_util.conv2d(f_max, d_out, [1, 1], name + 'max_pooling', [1, 1], 'VALID', True, is_training)
        # f_pc_agg = self.graph_att_pooling(f_concat, d_out, name + 'att_pooling', is_training)
        return f_pc_agg
    ...
    def gaussi(self, dist):
        return math.exp(-dist**2/2)

    def multi_head_attention(self, feature, d_out, name, is_training, n_heads=4):
        attns = []
        for i in range(n_heads):
            attention_features = self.get_atten_feature(feature, d_out//n_heads, name+'atten'+str(i), is_training)
            attns.append(attention_features)
        multi_head_f = tf.concat(attns, axis=-1)
        out = helper_tf_util.conv2d(multi_head_f, d_out, [1, 1], name + 'atten_mlp', [1, 1], 'VALID', True, is_training)
        return out

    def get_atten_feature(self, feature, d_out, name, is_training):

        Q = helper_tf_util.conv2d(feature, d_out, [1, 1], name + 'Q_mlp', [1, 1],
                                   'VALID', True, is_training)
        K = helper_tf_util.conv2d(feature, d_out, [1, 1], name + 'K_mlp', [1, 1],
                                  'VALID', True, is_training)
        V = helper_tf_util.conv2d(feature, d_out, [1, 1], name + 'V_mlp', [1, 1],
                                  'VALID', True, is_training)
        logits = tf.matmul(Q, K, transpose_b=True)
        weights = tf.nn.softmax(logits, axis=-1)
        if is_training is not None:
            weights = tf.nn.dropout(weights, 0.5)
        attention_features = tf.matmul(weights, V)
    ...
    @staticmethod
    def graph_att_pooling(feature_set, d_out, name, is_training):
